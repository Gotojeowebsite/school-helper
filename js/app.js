...
function resetPomodoro() {
  clearInterval(state.pomodoroTimer.intervalId);
  const settings = store.getState().pomodoro.settings;
  state.pomodoroTimer.isRunning = false;
  state.pomodoroTimer.isPaused = false;
  ...
  if (state.pomodoroTimer.timeRemaining > 0) {
    ...
    if (state.currentView === 'focus') {
      ...
      if (timeDisplay) {
        ...
        const progress = ((total - state.pomodoroTimer.timeRemaining) / total) * 100;
        const ring = document.getElementById('pomodoro-ring');
        if (ring) ring.style.background = `conic-gradient(var(--accent) ${progress}%, var(--bg-surface-hover) ${progress}%)`;
      }
    }
    
    if (state.pomodoroTimer.timeRemaining === 0 && window.AcademiaProNative) {
      window.AcademiaProNative.haptic('warning');
    }
  } else {
...
function completePomodoroPhase() {
  ...
  if (state.pomodoroTimer.currentPhase === 'work') {
    ...
    data.pomodoro.totalFocusMinutes += settings.workDuration;
    data.pomodoro.todayFocusMinutes += settings.workDuration;
    
    data.pomodoro.sessions.push({
      id: 'pom-' + Date.now(),
      startTime: new Date(Date.now() - settings.workDuration * 60000).toISOString(),
    ...
    if (window.AcademiaProNative) {
      window.AcademiaProNative.scheduleNotification('Focus session complete! Time for a break.', new Date(Date.now() + 1000));
    }
...
function updateDailyStreak(type = 'task') {
  ...
  let xpGain = 0;
  if (type === 'task') {
    data.streaks.dailyLog[today].tasksCompleted++;
    xpGain = 10;
  } else if (type === 'pomodoro') {
    data.streaks.dailyLog[today].focusMinutes += data.pomodoro.settings.workDuration;
    xpGain = 5;
  }
  
  data.streaks.dailyLog[today].xpEarned += xpGain;
  data.streaks.totalXP += xpGain;
  
  // Level up logic
  const newLevel = Math.floor(data.streaks.totalXP / 500) + 1;
  if (newLevel > data.streaks.level) {
    data.streaks.level = newLevel;
    showToast(`Level Up! You are now Level ${newLevel} 🎉`, 'success');
  }
  
  checkAchievements();
  store.notify('streak_update');
}
...
function checkAchievements() {
  ...
  const unlock = (id, title) => {
    if (!a.includes(id)) {
      ...
      if (window.AcademiaProNative) window.AcademiaProNative.haptic('success');
    }
  };
  
  if (data.streaks.totalXP > 0) unlock('first_task', 'Getting Started');
  if (data.streaks.currentStreak >= 3) unlock('streak_3', 'On a Roll');
  if (data.streaks.currentStreak >= 7) unlock('streak_7', 'Weekly Warrior');
  if (data.streaks.currentStreak >= 30) unlock('streak_30', 'Monthly Master');
  if (log.focusMinutes >= 60) unlock('focus_60', 'Deep Focus');
  
  let totalTasks = 0;
  for (let date in data.streaks.dailyLog) totalTasks += data.streaks.dailyLog[date].tasksCompleted;
  if (totalTasks >= 10) unlock('tasks_10', 'Task Crusher');
  if (totalTasks >= 50) unlock('tasks_50', 'Half Century');
}
...
function renderFocusTimerView() {
  ...
  const progress = ((total - timer.timeRemaining) / total) * 100;
  
  const phaseLabel = timer.currentPhase === 'work' ? 'Focus Session' : (timer.currentPhase === 'shortBreak' ? 'Short Break' : 'Long Break');
  
  container.innerHTML = `
    <div class="section-header">
      <h2 class="section-title">Focus Timer</h2>
    </div>
    <div class="card-panel" style="display:flex; flex-direction:column; align-items:center; padding: 3rem 1rem;">
      <div id="pomodoro-ring" class="pomodoro-timer-ring" style="background: conic-gradient(var(--accent) ${progress}%, var(--bg-surface-hover) ${progress}%);">
        <div class="pomodoro-timer-inner">
          ...
          <div class="pomodoro-counter">Session ${timer.sessionsCompleted % settings.sessionsBeforeLong + 1} of ${settings.sessionsBeforeLong}</div>
        </div>
      </div>
      <div class="pomodoro-controls" style="margin-top: 2rem; display:flex; gap: 1rem;">
        ${!timer.isRunning ? `<button class="btn-primary" onclick="startPomodoro()">Start</button>` : `<button class="btn-secondary" onclick="pausePomodoro()">Pause</button>`}
        <button class="btn-secondary" onclick="resetPomodoro()">Reset</button>
      </div>
    </div>
    
    <div class="stat-cards-grid" style="margin-top: 2rem;">
      <div class="stat-card">
        <div class="stat-icon-wrapper blue"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="12" cy="12" r="10"></circle><polyline points="12 6 12 12 16 14"></polyline></svg></div>
        <div class="stat-content">
          <div class="stat-label">Today's Focus</div>
          <div class="stat-value">${data.pomodoro.todayFocusMinutes} min</div>
        </div>
      </div>
      <div class="stat-card">
        <div class="stat-icon-wrapper emerald"><svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"></path><polyline points="22 4 12 14.01 9 11.01"></polyline></svg></div>
        <div class="stat-content">
          <div class="stat-label">Sessions Completed</div>
          <div class="stat-value">${data.pomodoro.sessions.length}</div>
        </div>
      </div>
    </div>
  `;
...
// Override renderAnalyticsView to include streaks
const originalRenderAnalyticsView = renderAnalyticsView;
renderAnalyticsView = function() {
  ...
  let heatmapHTML = '<div class="streak-heatmap" style="display:grid; grid-template-rows: repeat(7, 1fr); grid-auto-flow: column; gap: 4px; overflow-x: auto; padding: 1rem 0;">';
  const today = new Date();
  for (let i = 84; i >= 0; i--) {
    ...
    const intensity = log ? Math.min(4, Math.ceil((log.tasksCompleted + log.focusMinutes/10) / 2)) : 0;
    heatmapHTML += `<div class="heatmap-cell intensity-${intensity}" title="${dateStr}: ${log ? log.tasksCompleted + ' tasks, ' + log.focusMinutes + ' min' : 'No activity'}"></div>`;
  }
  heatmapHTML += '</div>';
  
  const badges = [
    { id: 'first_task', icon: '🚀', title: 'Getting Started' },
    { id: 'streak_3', icon: '🔥', title: 'On a Roll' },
    { id: 'streak_7', icon: '⚡', title: 'Weekly Warrior' },
    { id: 'streak_30', icon: '👑', title: 'Monthly Master' },
    { id: 'focus_60', icon: '🧠', title: 'Deep Focus' },
    { id: 'tasks_10', icon: '✅', title: 'Task Crusher' },
    { id: 'tasks_50', icon: '🌟', title: 'Half Century' }
  ];
  
  let badgesHTML = '<div style="display:flex; gap: 1rem; flex-wrap: wrap;">';
  badges.forEach(b => {
    const unlocked = streaks.achievements.includes(b.id);
    badgesHTML += `
      <div class="streak-badge" style="opacity: ${unlocked ? '1' : '0.4'}; filter: ${unlocked ? 'none' : 'grayscale(1)'}; background: var(--bg-surface-hover); padding: 1rem; border-radius: var(--radius-md); text-align: center; width: 100px;">
        <div style="font-size: 2rem;">${b.icon}</div>
        <div style="font-size: 0.75rem; font-weight: bold; margin-top: 0.5rem;">${b.title}</div>
      </div>
    `;
  });
  badgesHTML += '</div>';
  ...
  container.innerHTML = `
    ...
    <div class="dashboard-grid">
      <div class="card-panel">
        <h3 style="margin-bottom: 1rem;">Study Streak 🔥</h3>
        <div style="display:flex; justify-content: space-around; text-align: center;">
          <div>
            <div style="font-size: 2.5rem; font-weight: bold; color: var(--warning);">${streaks.currentStreak}</div>
            <div style="font-size: 0.875rem; color: var(--text-secondary);">Current Streak</div>
          </div>
          <div>
            <div style="font-size: 2.5rem; font-weight: bold; color: var(--text-primary);">${streaks.longestStreak}</div>
            <div style="font-size: 0.875rem; color: var(--text-secondary);">Longest Streak</div>
          </div>
        </div>
        
        <div style="margin-top: 2rem;">
          <div style="display:flex; justify-content: space-between; margin-bottom: 0.5rem;">
            <span style="font-weight: bold;">Level ${streaks.level}</span>
            <span style="color: var(--text-secondary);">${streaks.totalXP} / ${streaks.level * 500} XP</span>
          </div>
          <div class="xp-bar" style="height: 8px; background: var(--bg-surface-active); border-radius: 4px; overflow: hidden;">
            <div style="height: 100%; width: ${(streaks.totalXP % 500) / 5}%; background: var(--accent);"></div>
          </div>
        </div>
      </div>
      
      <div class="card-panel">
        <h3 style="margin-bottom: 1rem;">Activity Heatmap</h3>
        ${heatmapHTML}
      </div>
    </div>
...
// Hook into toggleAssignmentComplete
const origToggleAssignmentComplete = store.toggleAssignmentComplete;
store.toggleAssignmentComplete = function(id) {
  origToggleAssignmentComplete.call(this, id);
